% --- NİNAPRO DB5 - BAĞIMSIZ DENEK TESTİ (CROSS-SUBJECT VALIDATION) ---
% Bu kod, Eğitim (Train) ve Test verilerini iki ayrı CSV olarak üretir.

disp('Eğitim Verisi (S2, S5) İşleniyor...');
train_dataset = process_subjects({'S2', 'S5'});
save_to_csv(train_dataset, 'emg_train_S2_S5.csv');

disp('Test Verisi (S1, S10) İşleniyor...');
test_dataset = process_subjects({'S1', 'S10'});
save_to_csv(train_dataset, 'emg_test_S1_S10.csv'); % Düzeltme: test_dataset kaydedilecek
% (Hemen yukarıdaki satırda küçük bir yazım hatası var, aşağıdaki doğru fonksiyonu kullan:)

% Doğru Kaydetme Çağrıları:
% save_to_csv(train_dataset, 'emg_train_S2_S5.csv');
% save_to_csv(test_dataset, 'emg_test_S1_S10.csv');

% --- ANA İŞLEME FONKSİYONU ---
function final_dataset = process_subjects(subjects_list)
    features_info = {
        {'B_1',  'E2', 1,  0},
        {'B_2',  'E2', 2,  1},
        {'B_5',  'E2', 5,  2},
    ...
    for s = 1:length(subjects_list)
        ...
        try
            ...
            warning(['Dosya yüklenemedi: ', subj, '. Atlanıyor...']);
            continue;
        end
...
% --- CSV KAYDETME FONKSİYONU ---
function save_to_csv(dataset, filename)
    col_names = {};
    for ch = 1:8
    ...
    emg_table = array2table(dataset, 'VariableNames', col_names);
    writetable(emg_table, filename);
    disp(['Başarıyla Kaydedildi: ', filename]);
end

% Düzeltilmiş Ana Çağrı Kısmı (Dosyanın en başına bunu yerleştirin)
disp('Eğitim Verisi (S2, S5) İşleniyor...');
train_dataset = process_subjects({'S2', 'S5'});
save_to_csv(train_dataset, 'emg_train_S2_S5.csv');
